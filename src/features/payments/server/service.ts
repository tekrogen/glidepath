 ...
 */
import { daysUntil, type RunwayCard, type RunwayPayment } from "@/lib/finance"
import {
  findHouseholdCards,
...
/**
 * Move a SCHEDULED payment to a new date (issue #44). The date must not
 * be in the past — the runway window starts today, and back-dating a
 * planned payment records an intent that can no longer happen. A
...
export async function reschedulePaymentForUser(
  ...
  if (daysUntil(scheduledFor, today) < 0) {
    throw new Error("Cannot move a payment into the past")
  }
  const householdId = await findHouseholdIdForUser(userId)
