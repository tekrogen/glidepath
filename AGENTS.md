...
This file provides guidance to Codex (Codex.ai/code) when working with code in this repository. What follows are behavioral guidelines to reduce common LLM coding mistakes. Merge with project-specific instructions as needed.
...
5. **Agents** – the agents live in `/Volumes/SERV01-DTMAC/_Code_Library/AI Agents/` (additional agent definitions in `/Volumes/SERV01-DTMAC/_Code_Library/.Codex/agents/`)
...
- **Five projects**: `setup` (demo login → `tests/.auth/user.json`), `public` (no session), `authenticated` (read-only seed-exact specs, reuses storageState), `authenticated-mutations` (row-inserting specs; depends on `authenticated` so mutations run after the seed-exact assertions), `empty-state` (separate card-less user via `tests/.auth/empty.json`). Config: `playwright.config.ts`.
