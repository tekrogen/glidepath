...
test.describe("Shell architecture (mockup IA)", () => {
  ...
  test("brand lives in the sidebar; coming-soon surfaces are not navigable", async ({ page }) => {
    ...
    // Payments un-gated by #44 — it is a real link now; Rewards stays gated.
    const payments = page.getByTestId("primary-nav").locator("a", { hasText: "Payments" })
    await expect(payments).toHaveAttribute("href", "/payments")
    const rewards = page.getByTestId("primary-nav").locator("span[aria-disabled]", { hasText: "Rewards" })
